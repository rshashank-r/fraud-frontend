...
// Import existing modals/sections if needed, or keep them inline for now
// (Assuming we want to keep the functionality but start fresh with layout)
...
<div className="relative flex h-screen">
  ...
  <div className={`
    ...
    <div className="p-4 border-t border-slate-800">
      ...
      {sessionExpiresIn !== null && sessionExpiresIn > 0 && (
        <div className="flex items-center gap-2 bg-slate-700/50 px-3 py-2 rounded-lg mb-2">
          <Clock className="w-4 h-4 text-cyan-400" />
          <span className="text-xs text-gray-300">
            Session: {sessionExpiresIn}m
          </span>
        </div>
      )}
      <Button
  ...
  <div className={`flex-1 overflow-y-auto ${isMobile ? 'mt-16' : ''}`}>
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      ...
      {activeTab === 'overview' && (
        <Card className="mb-6 bg-gradient-to-br from-slate-800/50 to-slate-900/50 border-slate-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              ...
              <div className="flex items-center gap-4 p-4 bg-cyan-500/10 rounded-xl border border-cyan-500/20">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">Account: {user?.account_number}</p>
                </div>
              </div>

              {/* Trust Score Section */}
              <div className="flex items-center gap-4 p-4 bg-green-500/10 rounded-xl border border-green-500/20">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${(user?.trust_score || 0) > 80
                ...
                <div className="flex-1">
                  ...
                  <h2 className={`text-xl sm:text-2xl font-bold ${(user?.trust_score || 0) > 80 ? 'text-green-400' : 'text-yellow-400'
                    }`}>
              ...
              <div className={`flex items-center gap-4 p-4 rounded-xl border ${user?.is_locked
                ? 'bg-red-500/10 border-red-500/20'
                : 'bg-purple-500/10 border-purple-500/20'
                }`}>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${user?.is_locked
                ...
                <div className="flex-1">
                  ...
                  <h2 className={`text-xl sm:text-2xl font-bold ${user?.is_locked ? 'text-red-400' : 'text-green-400'
                    }`}>
